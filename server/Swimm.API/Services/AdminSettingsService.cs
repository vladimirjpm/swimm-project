...
public class AdminSettingsService
{
    ...
    private void SeedDefaults()
    {
        ...
        {
            // ── Maintenance mode ─────────────────────────────────────────
            // Если true — публичная часть сайта закрыта, показывается заглушка.
            // Админ-панель и /auth продолжают работать.
            // Админы видят сайт в обычном режиме.
            new("MaintenanceMode", "false", "bool", "livesite",
                "Режим обслуживания: true — сайт закрыт для всех кроме админов, показывается заглушка"),

            // ── Кеш схемы БД ────────────────────────────────────────────
            // Сколько минут хранить результат /api/admin/db-schema в памяти.
            // После истечения TTL следующий запрос выполнит все SQL-запросы заново.
            ...
            // Если true — кеш сбрасывается при КАЖДОМ запросе к db-schema.
            // По сути отключает кеширование. Полезно при активной разработке БД.
            new("ForceRefresh", "false", "bool", "admin",
                "Если true — кеш схемы сбрасывается при каждом запросе (отключает кеширование)"),

            // ── Системные таблицы ────────────────────────────────────────
            // PostgreSQL хранит внутренние объекты в схемах pg_catalog и information_schema.
            // По умолчанию они скрыты. Включите для отладки системных объектов.
            ...
            // ── Схема по умолчанию ───────────────────────────────────────
            // Фильтр table_schema в запросе таблиц.
