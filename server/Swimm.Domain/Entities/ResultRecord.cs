...
public class ResultRecord
{
    ...
    /// <summary>
    /// Клубные очки, которые за этот заплыв начислил САМ ОРГАНИЗАТОР (колонка «ניקוד קבוצתי»
    /// пособытийного источника loglig). null — источник таких очков не давал: у PDF-протоколов
    /// их нет вовсе.
    ///
    /// Это НЕ то, что мы показываем: зачёт по-прежнему считает наш движок правил
    /// (<see cref="Swimm.Infrastructure.Services.PointRulesClubsScoring"/>). Официальное
    /// значение лежит рядом как ЭТАЛОН — по нему проверка реестра
    /// <c>results.official-club-points-mismatch</c> находит строки, где мы разошлись с
    /// федерацией. Без построчного эталона расхождение видно только суммой, и разбирать его
    /// приходится раскопками (сверка 1581: −131 очко в 223 строках, разнонаправленно).
    ///
    /// Зачем вообще расходимся: организатор решает «кто получает очки» по СЕКЦИИ протокола,
    /// а не по раунду — у взрослых 19-99 платят мокдамот, секция «כללי» не платит вовсе.
    /// Вывести это из наших полей правилами нельзя (docs/data-integrity.md §10).
    /// </summary>
