...
public class CompetitionNote
{
    ...
    /// <summary>
    /// Табличка расхождения в JSON: <c>[{"place":21,"expected":5,"actual":6,"subject":"…"}, …]</c>
    /// — место, сколько по регламенту, сколько начислили официально и КОМУ достались очки.
    /// null — расхождение объясняется только прозой.
    ///
    ...
    /// <summary>
    /// Подпись таблички — КАКОЙ заплыв разобран в примере: «50 m backstroke · girls 17-18 ·
    /// final». Без неё столбик мест висит в воздухе: читатель не знает, к какой дистанции
    /// он относится, а расхождение всегда живёт в конкретном заплыве.
    ///
    /// Одна на все языки, как ссылка и цифры: названия дисциплин на сайте и так английские.
    /// </summary>
    [MaxLength(200)]
    public string? ScaleDiffCaption { get; set; }
