...
## История: был обрыв на несоскоупленных запросах — УСТРАНЁН

Изначально этот профиль вскрыл обрыв (2026-07-14, 20 VU, 3.01M SYNTH, воспроизведено):
`global-filtered` p95 **~14 с**, max 23 с.

**Корень:** `/api/results` фильтровал стиль по `Style.Name` через JOIN. Это мешало
планировщику PostgreSQL взять уже существующие композитные индексы
`IX_Results_StyleId_Distance_Gender_CompDate` (фильтр + `ORDER BY CompetitionDate`) и
`IX_Results_CompetitionId_StyleId_Distance_Gender` (для `COUNT`); он шёл backward-scan'ом
по `IX_Results_CompetitionDate` с inline-фильтром + отдельным `COUNT` с JOIN Styles. Одиночный
запрос — 20–140мс (curl, тёплый план), но **20 конкурентных промахов насыщали CPU/IO → очередь
→ секунды**. EXPLAIN на 3М: LIMIT 34→4 мс, COUNT 86→4 мс (JOIN vs StyleId).

**Фикс (код, без миграции — индексы уже были):** `ResultRepository` резолвит `styleName→StyleId`
через кэш (`ResolveStyleIdsAsync`, Styles = 8 строк, TTL 10 мин) и фильтрует по `StyleId`, а не
`Style.Name`. Планировщик берёт композитные индексы. Под теми же 20 VU:

| Группа           | p(95) ДО   | p(95) ПОСЛЕ | Порог  |
|------------------|-----------|------------|--------|
| global-list      | ~3.6 ms   | ~6 ms      | <300ms ✓ |
| global-filtered  | **~14 s** | **~5.5 ms** | <300ms ✓ |
| global-athlete   | 2.6мс–4.4с | ~4.8 ms   | <300ms ✓ |

Пропускная способность 4.4 → 451 итер/с (~100×), 565k запросов за прогон, `http_req_failed`
0.14% (транспортные сбросы под экстремальной нагрузкой, 0 исключений уровня приложения).
**Теперь это настоящий green-gate по 300мс на всей таблице 3М.**

**Остаточное (не блокер, за рамками):** `/api/athletes/career` сканирует по имени без индекса —
холодный первый скан ~5с на уникальном имени (max/p99), тёплый — мс (кэш 5 мин). Кандидат на
индекс по имени, если понадобится.
