    ...
    public async Task<MySwimsResponseDto> GetMySwimsAsync(int userId, int? season, bool allSeasons = false)
    {
        ...
        var swims = await _db.Results
            ...
            .Select(r => new MySwimDto
            {
                ...
                // Место есть только там, где его РЕАЛЬНО заняли, — то же правило, что у
                // медального зачёта (docs/competition-overview-cards.md, «Что считается
                // медалью»; решения Р34 и Р43). Строка My media рисует медаль из места, и
                // без этих трёх срезов она обещала награду, которой не вручали:
                //  • prelim / extra — ранжир сессии, а не места: у бугрим протокол печатает
                //    места и в предварительных, иначе «первый утром и вечером» дал бы два
                //    золота за одну дисциплину;
                //  • final-open («כללי») — секция без возрастной категории: ни очков, ни
                //    медалей, свои награды эти пловцы получают в возрастной секции;
                //  • TimeFail (DQ / NS / DNS) — снятому места нет, даже если протокол
                //    напечатал число (инцидент И-18, docs/data-integrity.md).
                // Так же режут Position таблица результатов, страница клуба, хаб-группы и
                // клубный зачёт — My media была последней, кто отдавал его сырым.
                Place = r.HeatType == "prelim" || r.HeatType == "extra"
                        || r.Round == ResultRounds.FinalOpen || r.TimeFail
                    ? null
                    : r.Position,
