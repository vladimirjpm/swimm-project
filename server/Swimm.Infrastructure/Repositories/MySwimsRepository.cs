    ...
    public async Task<MySwimsResponseDto> GetMySwimsAsync(int userId, int? season, bool allSeasons = false)
    {
        ...
        var swims = await _db.Results
            ...
            .Select(r => new MySwimDto
            {
                ...
                // Место отдаём КАК В ПРОТОКОЛЕ, включая предварительные: пловцу важно
                // видеть, что утром он был первым (решение Влада 08.09.2026). А вот МЕДАЛЬ
                // из этого числа выводить нельзя — она считается по правилу медального
                // зачёта (docs/competition-overview-cards.md, решения Р34 и Р43):
                //  • prelim / extra — ранжир сессии: у бугрим протокол печатает места и в
                //    предварительных, иначе «первый утром и вечером» дал бы два золота за
                //    одну дисциплину; строка помечается `[prelim]`;
                //  • final-open («כללי») — секция без возрастной категории: ни очков, ни
                //    медалей, свою награду эти пловцы получают в возрастной секции;
                //  • TimeFail (DQ / NS / DNS) — снятый награды не получает.
