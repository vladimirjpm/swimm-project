...
public class HubGroupPublicRepository : IHubGroupPublicRepository
{
    ...
    public async Task<HubGroupDetailsDto?> GetPageAsync(int groupId, string slug)
    {
        ...
        // только контекст записи. Таблица админов — целиком (меняется редко). Пользователи —
        // ТОЛЬКО строки владельца и админов (корень сужения AppUser): таблицей страницы всех
        // групп падали бы на каждую регистрацию и правку любого пользователя (К4б.0). Отметка
        // «был онлайн» (LastSeenAt, ExecuteUpdate) кэш не роняет вовсе.
        var adminUserIds = await _rw.HubGroupAdmins.AsNoTracking()
            .Where(a => a.HubGroupId == groupId)
            .Select(a => a.UserId)
            .ToListAsync();
        adminUserIds.Add(group.OwnerUserId);
        List<int> adminSwimmerIds;
        // ⚠ Правило блока: каждый запрос в нём фильтрует пользователей по этим id.
        using (_rw.CacheRows<AppUser>(adminUserIds))
        {
            adminSwimmerIds = await _rw.AppUsers.AsNoTracking()
