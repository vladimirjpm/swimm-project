...
/// соревнование, стиль, дистанция, пол, заплыв, дорожка, эстафета?, РАУНД) совпадает МАССОВО
/// и это норма: заплывы нумеруются внутри возрастной ступени, поэтому «100 комплекс, заплыв 1,
...
/// скопирован и живёт своей жизнью. Меняешь ключ матчера — правь здесь же.
/// </summary>
public sealed class UpsertKeyCollisionCheck(SwimmDbContext db) : IDataCheck
{
    ...
    public string Description =>
        "У одного пловца две строки с одинаковыми соревнованием, дисциплиной, полом, заплывом, " +
        "дорожкой и раундом — переимпорт не может сопоставить их с файлом по отдельности. Бывает " +
        "законно (протокол печатает и снятие, и результат), но при «удалять лишние» такая пара " +
        "ведёт себя непредсказуемо. Ключ upsert не различает программы (EventCategory).";
