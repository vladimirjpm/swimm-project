...
public class RegulationAnalyzer : IRegulationAnalyzer
{
    ...
    /// («50 ,'מ  10  םילולסמ» — лига длинных бассейнов, takanon 14575).
    /// </summary>
    private static readonly Regex[] PoolRx =
    [
        new(@"(?<!\d)(?<len>25|50)\s*(?:מטר|מ['׳])\s*[,\-–]?\s*\d{1,2}\s*מסלולים", RegexOptions.Compiled),
        new(@"(?<!\d)(?<len>25|50)\s*,?\s*(?:רטמ|['׳]מ)\s*\d{1,2}\s*[.,]?\s*םילולסמ", RegexOptions.Compiled),
    ];
