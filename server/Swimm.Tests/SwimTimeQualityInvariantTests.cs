...
public class SwimTimeQualityInvariantTests
{
    ...
    private static readonly Dictionary<string, string> Exempt = new()
    {
        ...
        ["RecordSuspiciousEntry"] = "находка сторожа импорта рекордов — будущая претензия (FlaggedTime): время в ней и есть оспариваемое значение, причина — полем Reason",
        ...
        ["RecordCountryMismatchDto"] = "строка отчёта источника, которую прогон НЕ принял (код федерации не равен запрошенной стране): в БД её нет и не будет, время приведено уликой расхождения",
        ...
        // ── Цитата из протокола внутри объяснения, а не показ результата: строка приводится
        //    как доказательство расхождения ОЧКОВ, и время в ней — опорная точка («места
        //    расставлены верно, спорны начисления»), а не достижение, которое мы оцениваем.
        ["ScaleDiffRowDto"] = "строка таблички расхождения очков — цитата протокола, оспариваются очки, не время",
        ...
        // ── Доклейка промежуточных (SplitAttachMatcher): вход и выход ЧИСТОЙ функции,
        //    наружу не отдаются. Время в них — ключ сопоставления «эта команда = эта строка
        //    базы», а не показываемый результат; качество живёт на самой строке Results.
        ["SplitSourceTeam"] = "команда из источника loglig — ключ сопоставления, не витрина",
        ["SplitSourceSwim"] = "личный заплыв из источника loglig — ключ сопоставления, не витрина",
        ["RelayRow"] = "строка эстафеты из БД как кандидат доклейки — служебный вход матчера",
        ["SwimRow"] = "строка личного заплыва из БД как кандидат доклейки — служебный вход матчера",
    };
