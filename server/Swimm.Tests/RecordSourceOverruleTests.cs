...
///
/// Закрывает дыру, которую метка в реестре не закрывает: метка метит ЗНАЧЕНИЕ, а Apply всё
/// равно перезаписал бы верное число неверным. Потеряй источник рекорд — следующий импорт
/// молча заменил бы его худшим, и до исправления файла федерацией правильного значения у нас
/// бы не было.
///
/// ⚠ Механизм ПРОФИЛАКТИЧЕСКИЙ: живого случая нет. Заказан он был, пока находка на 17 ж
/// 50 в/с 50 м читалась как потеря рекорда; разбор по КАЛЕНДАРНОЙ оси справочника показал,
/// что это, наоборот, починка дубля источником, и обе претензии закрыты как `rejected`
/// (docs/data-integrity.md И-25). Числа ниже — из того разбора, и как образец «источник
/// отдал время медленнее нашего» они верны независимо от того, кто в том споре был прав.
///
...
public class RecordSourceOverruleTests
{
    ...
    /// <summary>Строка, которая лежит у нас: 17 ж 50 в/с 50 м.</summary>
    ...
    /// <summary>То, что приехало из источника, — медленнее на 0.20.</summary>
    ...
    private static RecordIssue IssueOn(string time, string status) => new()
    {
        ...
        Note = "Человек разобрал и признал это значение ошибкой источника.",
        CreatedBy = "vlad",
    };
    ...
    /// как раз правильное. Ровно этот статус получили ОБЕ живые находки 17.09.2026 (источник
    /// дважды убрал свой же дубль), и их импорт обязан применять.
    /// </summary>
    [Fact]
    ...
    /// новый, ещё более быстрый рекорд — защита его не держит.
    /// </summary>
    [Fact]
