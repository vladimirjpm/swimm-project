...
public sealed record ImportEventOptions(
    ...
    // ── Флаги соревнования из превью затягивания ──────────────────────────────
    // Все nullable: null = «не трогать» (парсер/существующая запись решают сами),
    // значение = решение человека в превью, оно главнее. Раньше их проставляли РУКАМИ
    // после импорта, в панели строки, и про них забывали.

    /// <summary>Вручают медали (Awards). Предлагается по регламенту (מדליות).</summary>
    bool? IsAward = null,

    /// <summary>Чемпионат Израиля (🏆). Предлагается по названию и по регламенту.</summary>
    bool? IsChampionship = null,

    /// <summary>Мастерс-соревнование. Предлагается по разобранному файлу.</summary>
    bool? IsMasters = null,

    /// <summary>Клубный зачёт не ведётся. Предлагается, когда loglig показал «зачёта нет».</summary>
    bool? ClubPointsDisabled = null,

    /// <summary>
    /// Длина бассейна («25m»/«50m»). Переопределяет то, что распознал парсер, ДО построения
    /// ключа соревнования — бассейн входит в уникальный ключ (Name|Date|PoolType), и менять
    /// его после импорта значит плодить дубль.
    /// </summary>
