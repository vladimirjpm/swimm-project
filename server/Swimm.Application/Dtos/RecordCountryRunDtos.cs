...
/// <summary>
/// Состояние прогона рекордов по странам (этап 11.1.2).
///
/// ⚠ В JSON уходит СТРОКОЙ, а не числом. По умолчанию System.Text.Json отдал бы
/// «state: 2», и на той стороне это молча не совпадало ни с чем: админка сравнивала
/// с «Completed», ветка не срабатывала, опрос прекращался после первого ответа, а
/// прогон при этом шёл и заканчивался нормально — поломка без единой ошибки в консоли
/// (поймано 16.09.2026 при сборке вкладки). Числовой enum в JSON-контракте — ловушка
/// именно такого рода: ошибиться легко, заметить нечем.
/// </summary>
