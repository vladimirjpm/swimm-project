 ...
 *
 * «Сторону нельзя сменить» (в табе левый — хозяин страницы) выражается через `onSelect: null`,
...
export type H2HSlot =
  | {
      ...
      /**
       * Выбрать на эту сторону другого пловца — клик по самой карточке. null — слот
       * несменяемый (в табе левый это хозяин страницы), и карточка не кнопка.
       */
      onSelect?: (() => void) | null;
  ...
  | {
      ...
      onPick?: () => void;
      /**
       * Эту сторону сейчас заполняет пикер: слот подсвечивается акцентной рамкой.
       * Нужен там, где пикер ОДИН на два слота (страница `/h2h`) — подпись «choosing the
       * left swimmer» жила отдельно от слотов, и связь читалась только по тексту.
       * В табе пикер всегда ищет соперника, поэтому флаг там не передаётся вовсе.
       */
      active?: boolean;
    };
