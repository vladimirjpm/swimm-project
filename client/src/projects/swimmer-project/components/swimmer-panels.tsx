            ...
            {progress.points.map((p) => (
              <SwimmerResultRow
                ...
                row={{
                  ...
                  round: p.round,
                  timeFail: p.timeFail,
                  competitionIsAward: p.competitionIsAward,
...
export function H2HPanel({
  ...
  const right: H2HSlot = rivalId == null
    ...
    : {
      ...
      ...favProps(rivalId),
      // Клик по карточке соперника — «выбрать другого»: освобождаем сторону и сразу
      // уводим курсор в поиск.
      onSelect: () => { onClear(); window.setTimeout(() => searchRef.current?.focus(), 0); },
    };

  return (
    <>
      {/* Кнопки «Clear» в шапке нет: сменить соперника можно кликом по его карточке —
