...
export interface MySwimDto {
  ...
  place: number | null;
  /** Место, за которое дают медаль (1–3 в зачётном заплыве). Медаль рисуется ТОЛЬКО по нему:
   *  у предварительных, у секции «כללי» и у снятых места есть, а награды нет. */
  is_award: boolean;
  /** `prelim` / `final` / … — для пометки `[prelim]` у места. */
