  ...
  return (
    <div className="mt-[18px] flex flex-col gap-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="ml-auto inline-flex overflow-hidden rounded-[10px] border border-[var(--t-accent-border)]">
      ...
      {visible.length === 0 ? (
        <div className="rounded-[16px] border border-dashed border-[var(--t-accent-border)] p-12 text-center">
          <p className="m-0 text-[15px] font-extrabold text-[var(--t-accent)]">No pending requests 🎉</p>
        </div>
      ) : (
        <div className="flex flex-col gap-2">
          {visible.map((r) => {
            ...
            return (
              ...
              >
                <div
                  className="flex aspect-video w-[88px] shrink-0 cursor-pointer items-center justify-center rounded-[8px] bg-[var(--t-surface2)] text-[13px] text-[var(--t-accent)]"
                ...
                <span
                  ...
                  style={
                    r.level === 'public'
                      ? { color: 'var(--t-warn)', border: '1.5px solid var(--t-warn-border)', background: 'var(--t-warn-soft)' }
                      : { color: 'var(--t-text-2)', border: '1px solid var(--t-border)', background: 'var(--t-surface2)' }
                  }
                >
                  {r.level === 'public' ? 'Public 🌐' : 'Members'}
                </span>
                <span className="hp-mono w-14 shrink-0 text-[10.5px] text-[var(--t-text-3)]">
                ...
                <div className="flex shrink-0 gap-1.5">
                  {r.status === 'pending' && (
                    <>
                      <button
                        ...
                        className="hp-mono rounded-[8px] border-none bg-[var(--t-accent)] px-3 py-[6px] text-[11.5px] font-extrabold text-[var(--t-accent-ink)] disabled:opacity-50"
                      >
                        Publish
                      </button>
                      <button
                        ...
                        className="hp-mono rounded-[8px] border border-[var(--t-danger-border)] bg-transparent px-3 py-[6px] text-[11.5px] font-extrabold text-[var(--t-danger)] disabled:opacity-50"
                      >
                        Reject
                      </button>
                    </>
                  ...
                  {r.status === 'approved' && (
                    <button
                      ...
                      disabled={busyId === r.id}
                      onClick={() => decide(r, false)}
                      className="hp-mono rounded-[8px] border border-[var(--t-accent-border)] bg-transparent px-3 py-[6px] text-[11.5px] font-extrabold text-[var(--t-accent)] disabled:opacity-50"
                    >
                      Unpublish
                    </button>
                  )}
      ...
      <p className="m-0 text-[11px] italic text-[var(--t-text-3)]">
