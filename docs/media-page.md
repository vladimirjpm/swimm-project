...
4. **Метки достижений — общий словарь продукта, локальных правил тут НЕТ.**
   ...
   одно время.
4б. **Место и медаль в строке.** Медаль в строке рисуется ИЗ МЕСТА (`place <= 3`), отдельного
   признака награды в данных нет. Поэтому агрегат отдаёт место только там, где его реально
   заняли, — тем же правилом, что медальный зачёт соревнования
   (`docs/competition-overview-cards.md`, «Что считается медалью»): не отдаём место у
   `prelim`/`extra` (ранжир сессии, решение Р34), у `final-open` — секции «כללי» без
   возрастной категории (Р43) и у `TimeFail` (DQ / NS / DNS, инцидент И-18). До 08.09.2026
   My media была единственным экраном, который отдавал `Position` сырым, и строка показывала
   «#2 🥈» рядом со словом DSQ.
