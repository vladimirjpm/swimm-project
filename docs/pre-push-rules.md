...
|---|---|---|---|
| 1 | **кэш**: `ICacheService`, `MemoryCacheService`, `CachedJsonExtensions`, `CacheTags`, `CacheRowRoots`, `CacheRowsExtensions`, `CacheSettings`, `CacheValueRules`, `CacheBuildScope`, `CacheDependencyInterceptor`, `CacheInvalidationInterceptor`, `ICacheDiagnostics`, `CacheOverview`, `_CacheReset.cshtml`, константы `Cache-Control`/`CacheControlValue`, TTL (`PayloadTtl`, `*Ttl`), сброс (`InvalidateAllAsync`/`InvalidateTagsAsync`/`InvalidateTableCacheAsync`) | тексты `/Admin/Cache` (`Pages/Admin/Cache.cshtml`), карта `WhereOnSite` в `Cache.cshtml.cs`, [admin-pages/cache.md](admin-pages/cache.md); блок «Кэш» дашборда — [admin-pages/index.md](admin-pages/index.md); правила или этапы кэша — [plans/cache-tags-plan.md](plans/cache-tags-plan.md) | решение Влада 11.09.2026 |
...
| 11 | **боевой прогон рекордов** (`--records-refresh` с Apply, не `--dry-run`) | файлы источников с НОВОЙ контрольной суммой + пересобранные CSV (`--records-dump`) в `!records-sources/`, обновлённый `SHA256SUMS.txt`; итог прогона — в [data-integrity.md](data-integrity.md) | решение Влада 16.09.2026, И-21 |
| 11-бис | **боевой прогон по странам** (кнопка «Прогнать все страны») | выгрузку прогон пишет САМ — `!records-sources/worldrecords-countries__fetched-<дата>.csv`, путь показан в админке под прогрессом. От тебя: закоммитить файл, дописать строку в `SHA256SUMS.txt`, итог прогона — в [data-integrity.md](data-integrity.md). ⚠ Прогон ПОДМНОЖЕСТВОМ стран не архивируется: это отладка | решение Влада 17.09.2026, И-22 |
| 12 | новый экран, где показан справочник `Records` (рекорды из импорта) | подпись `UI_RecordsChecked` с источниками ЭТОГО экрана (`recordSources(rows)`); новый тип проверки — ключ в `RecordSources.FreshnessKeys` и строка в журнале от того, кто его прогоняет | решение Влада 23.09.2026, records-freshness-plan |
