...
Статус: **модель пересмотрена и 2B′ РЕАЛИЗОВАН** (2026-07-14, см. §10). Публичный UGC-слой
(старый 2B) отменён решением Влада — вместо него group-scoped members-слой. 2A (личное
owner-only медиа в Sys_UserMedia) — отложен, кандидат на делегирование Сонету.
Предшественник: Phase 1 (favorites) сделан и живёт. Контекст — memory `favorites-media-feature`,
правило `rule-primary-favorite-untrusted`.
...
- UI выбора konkретного заплыва (result_id) в редакторе НЕ делался — сервер поддерживает,
  UX-питание отложено до запроса.
...
- 2A: личное owner-only медиа (Sys_UserMedia, MediaController api/me/media) — таск Сонету.
- Выбор заплыва в редакторе разборов (сервер готов, нет UX).
- youtube-nocookie + sandbox-атрибут в общем лайтбоксе UI_SwimmerGallery (усиление §4) — общий
  компонент, менять отдельным коммитом.
