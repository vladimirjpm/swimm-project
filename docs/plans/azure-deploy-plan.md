...
`server/db/seed/records/`, **не** в `client/public` — иначе снова уедут в dist).
~~И дописать `isrorg-masters` в порядок `--records-refresh`~~ — **сделано 2026-09-16**, источник
в списке (data-integrity.md, раздел про `--records-refresh`). ⚠ Мастерсы 25 м он всё равно не
приносит: ссылка на короткий PDF у федерации битая (И-15), нужен ручной перехват URL.
