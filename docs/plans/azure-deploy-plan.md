...
| Р5 | Главная страница прода — **клиентская** (EN, React). Серверная удаляется |
...
затрёт серверный. ✅ Решено (Р5): остаётся **клиентская**, серверная удаляется вместе со строкой
`@source` в `Styles/admin.css`. Судьба баннера «БД недоступна» — §10-В2.
...
`swimmer.html`, `club.html`, `groups.html`, `media.html`.
⚠ **Не закрыты `/results` и `/competitions/{id}`** — они отдаются из `results_main.html` и тоже
показывают поимённые результаты. Нужно решение (§10-В3-бис).
...
### A4. БД: роли, миграции, гранты ☐ `[код]` — закрывает Б9
...
### A5. Наполнение прод-данных ☐ — закрывает Б11
...
### A6. CI/CD ☐ `[код]` + `[Влад]` — закрывает Б4
...
**В3-бис. Закрывать ли от индексации ещё и `/results` с `/competitions/{id}`?** Решение В3
выполнено для страниц пловца, клуба, групп и медиа. Но список результатов отдаётся из
`results_main.html` и показывает ровно те же имена и годы рождения. Формально это за рамками
принятого решения, поэтому молча расширять не стал.
→ **Рекомендую закрыть тоже** — иначе смысл В3 теряется наполовину.
...
**В13. Деплой:** publish-profile (проще, начать с него) или OIDC (правильнее, нужны права на
App Registration в Entra ID)? Автоматически на push в master или только по кнопке?
→ **Рекомендую: publish-profile + только по кнопке**, пока не пройдена приёмка A8.
