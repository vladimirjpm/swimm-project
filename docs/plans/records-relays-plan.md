...
- **Следующее:** хвосты §5 (`none` у эстафет закрыт 22.09, И-32); `M`/`F` в карточках закрыт 22.09; 25m у טריילז закрыт 22.09 (И-30); строка эстафетного рекорда закрыта 22.09 — хвосты §5 кончились.
...
- ~~104 эстафеты с `none`~~ — **закрыто 22.09**: старый импорт юниоров 16803, мастерс 16743 до
  сборщика и 5 строк микста; переимпорт + две починки, `none` у эстафет 0 — И-32 в `data-integrity.md`.
...
- ~~18 карточек пловцов с `M`/`F`~~ — **закрыто 22.09**: писал сидер тренировок Дельфина (все 18 — `local`); миграция `SwimmerGenderMaleFemale` перевела данные и поставила `CK_Swimmers_Gender` (male/female/NULL); импорт сводит пол карточки через `RelayGender.Normalize`.
- ~~Откуда 25m у «טריילז»~~ — **закрыто 22.09**: умолчание парсера выдавалось за распознанное; бассейн теперь из регламента, новое соревнование без выбора не импортируется (И-30). 1566 и 1549/1550 → 50m по регламентам.
- ~~Строка эстафетного рекорда на странице пловца~~ — **закрыто 22.09**: слева клуб-держатель
  (`Records.Club` → `relayTeam`), бейдж «Relay team», состав — в подсказке (`relayHolders`).
