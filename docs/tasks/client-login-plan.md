...
## Шаг 1. `useAuth`-хук + user-меню в шапке (4.1) — **Fable**
...
## Шаг 2. Модал логина (4.2) — **Fable** (вместе с шагом 1, один коммит)
...
## Шаг 3. Регистрация + «забыл пароль» в модале (4.3) — **Sonnet** (после шага 2)
...
## Шаг 4. Прод-`IEmailSender` (4.4) — **Fable** (секреты/прод-конфиг)
...
## Шаг 5. CTA «залогинься» в Favorites-UI (4.5) — **Sonnet** (мелкий)
...
из UI клиента без curl. Плюс наш локальный мотив: залогиниться в browser-preview и
