...
> ⚠ **ВЫПОЛНЕНО, файл — архив** (см. [`README.md`](README.md)). Paged-режим работает:
> фильтры уходят на сервер из `filter-data-source-ddl.tsx`, фаза 3 закрыта целиком.
> Написанное ниже «сейчас работает как full» и «TODO в `loadFromApi`» — состояние на день
> постановки задачи, не сегодняшнее.
