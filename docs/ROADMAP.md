...
- ⏳ 3.6. (план 11.09.2026; **К1–К4 сделаны** — метки ставятся сами, склейка промахов, правила Redis тестом, сброс по меткам сам после коммита (14.09); дальше К5 guard-тест и К6 Redis) **Сброс кэша по меткам + готовность к Redis** — вместо
  ...
  строки — решено делать (14.09.2026); К4б.0–К4б.6 сделаны (лишние зависимости страницы группы,
  журнал сбросов на /Admin/Cache — PR #29 поверх PR #28 с К4; метки строк при записи — PR #30;
  сужение при чтении со сверкой на попадании и выключателем — PR #31 поверх #30; страница группы
  на блоках сужения — PR #32 поверх #31; клуб и выключатель включён по умолчанию — PR #33 поверх
  #32; служебные колонки — loglig и проверка качества больше не роняют витрины, выключатель
  включён по умолчанию — PR #34 поверх #33), остался К4б.7 (пловец) — только по замеру на проде —
