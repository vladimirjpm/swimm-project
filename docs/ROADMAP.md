...
- ⏳ 3.6. (план 11.09.2026; **К1–К4 сделаны** — метки ставятся сами, склейка промахов, правила Redis тестом, сброс по меткам сам после коммита (14.09); дальше К5 guard-тест и К6 Redis) **Сброс кэша по меткам + готовность к Redis** — вместо
  ...
  строки — решено делать (14.09.2026); К4б.0–К4б.3 сделаны (лишние зависимости страницы группы,
  журнал сбросов на /Admin/Cache — PR #29 поверх PR #28 с К4; метки строк при записи — PR #30;
  сужение при чтении со сверкой на попадании и выключателем — ветка `cache-k4b-03` поверх #30),
  дальше К4б.4 (страница группы) — [plans/cache-row-precision-plan.md](plans/cache-row-precision-plan.md).
