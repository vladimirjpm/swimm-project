...
- ☐ 11.1.3. **Качество кодов регионов.** Парсер выводит `RegionCode` не из запрошенного
  ...
  несовпадений вместо молчаливой записи. *Причина `USA`/`JAM` найдена и починена 2026-09-15:*
  повторённый мировой рекорд `=WR` парсер считал национальным (data-integrity И-19). Там же
  найдено, что источник сам отдаёт невозможное время (И-20) — сторож правдоподобия сделан
  2026-09-15: находки идут в реестр спорных рекордов кандидатами (records-quality-plan §3-бис).
...
3. **Израиль в батче** — пропускать или гнать федерацию после батча (рекомендация плана —
   пропускать).
