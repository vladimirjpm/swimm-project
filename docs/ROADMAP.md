...
- ☐ 11.1.2. **Батч-фетч (только NR, `open`).** ~200 стран × 2 бассейна (SCM/LCM) =
  ...
  **Второй инвариант (2026-09-15): Израиль в батч не входит** — на `country/ISR/open`
  федерация обязана применяться ПОСЛЕ World Aquatics (И-13), иначе молча откатятся 19
  рекордов; Израиль обновляется только `--records-refresh`.
