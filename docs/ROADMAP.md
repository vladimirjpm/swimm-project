...
**сезонный агрегат** (сезон = 1 сен–31 авг, метка по году начала) и «лучшее в сезоне»
поверх `ResultRecord`. Считать его дважды по-разному нельзя.

- ☐ 10.1. **Сезонный агрегат — общий шов.** Хелпер «результаты → сезоны» в одном месте
  (границы сезона, best-in-season, PB-детекция). Исключать `TimeFail` (DSQ/DNS) и
  `SuspectReason` из best/PB. Ключ дисциплины — стиль × дистанция × **бассейн**
  (25m/50m несравнимы) и, для страницы спортсмена, `EventCategory`.
- ☐ 10.2. **Спортсмен: `GET /api/athletes/{id}/timeline`** — `seasons[]` (соревнования,
  заплывы, очки, медали, подиумы, число личников, пик уровня, клуб того сезона) +
  `byEvent[].seasonBests[]` с дельтой к прошлому сезону и флагом PB.
  Попутно: `/api/athletes/career` ищет **по имени, не по id** — решить (id как основной ключ).
- ☐ 10.3. **Страница спортсмена** до уровня попапа и выше: суперлативы, графики по сезонам,
  матрица season-bests, история заплывов с фильтрами, рекорды, медиа-галерея, группы.
