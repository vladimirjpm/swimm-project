...
- ✅ **Эстафеты в справочнике рекордов (`Records`)** — решение Влада 21.09.2026; Э0–Э6 сделаны
  22.09.2026 (ветка records-freshness). План и итог — [plans/records-relays-plan.md](plans/records-relays-plan.md).
  Пол `mixed` (смешанная эстафета) в `Records` и `Results`, `none` = пол неизвестен; WJR-эстафеты
  (однополые и смешанные, полоса 14–18); смешанные WR/NR/федерации; метки рекордов у эстафет
  в протоколе и карточке «New records»; кнопка «Mixed» на `/records`; эстафетные рекорды команды
  на странице пловца. Эстафеты мастерс не берём (другая ось). Хвосты §5 плана (104 эстафеты с
  `none`, M/F в карточках, бассейн у טריילז, команда в рекорде пловца) закрыты 22.09 — PR #88.
...
- ✅ 8.14. (2026-09-26, L1–L5) Уровни пловцов и план дорожек: у группы свои
  уровни (4 стандартных, правятся), тренер ставит уровень каждому пловцу; план на дату —
  дорожки с уровнем и заданием, кнопка «Distribute» раскладывает пловцов по уровню, дальше
  руками; опубликованный план видят только участники (таб `Lanes`, подсветка «Your lane»).
  Всё приватное (`Sys_`). «Copy from previous», склейка пловцов и перечень потерь при
  удалении группы знают новые таблицы. «Auto lanes» (26.09) — дорожки между уровнями по числу
  пришедших, соседние уровни при нехватке дорожек вместе. Этапы L1–L5 — [`plans/lane-plans-plan.md`](plans/lane-plans-plan.md).
...
- ✅ 9.9. (2026-09-22) UI-переключатель региона рекордов — выбор региона в табе **WR** на
  `/records` (решение Влада 22.09.2026): World — мировые рекорды, как было; страна — все её
  национальные рекорды бассейна и пола, колонка «vs WR» с отставанием от мирового (на мобайле —
  на месте даты), дисциплина ведёт в рейтинг стран с этой страной подсвеченной. Регион в адресе
  `?region=USA`. Только open: возрастные и мастерские рекорды есть лишь у Израиля — их табы
  не переключаются.
...
9.9 сделан 22.09.2026; 9.8 — по продуктовому поводу.**
...
Фаза 9 (страны) — сквозная; 9.1–9.7 и 9.9 сделаны, 9.8 ждёт продуктового повода
