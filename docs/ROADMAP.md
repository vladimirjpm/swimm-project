...
- ☐ 8.7. Официальные клубные группы: `HubGroup.IsOfficial` (одна на клуб), заявки
  ...
  бейдж «Official Group of {Club}». **В работе** (uncommitted).

**Критерий приёмки (8.1–8.6, выполнено):** группа создаётся в админке и пользователем по правам,
публичная страница показывает состав/рекорды/заплывы/сезонный зачёт, избранное работает как
личная группа, со-тренеры управляют чужой группой в рамках прав.
