...
- ✅ 3.5. Нагрузочный smoke: k6-профиль «список + 3 фильтра + карточка спортсмена»
  ...
  код выхода 99).
  - **Строгий бенчмарк по всей таблице** (`server/loadtest/full-scan-smoke.js`, 2026-07-14):
    несоскоупленные запросы (без competitionId, произвольные фильтры) под 20 конкурентными VU
    со свежими кэш-ключами дают **p95 ~14 с** на `global-filtered` (COUNT+scan по подмножеству
    3М без покрывающего индекса, контеншен кладёт БД). Соскоупленный путь (реальное UI-usage
    по источнику) — 6 мс, обрыва нет. Задокументировано как ИЗВЕСТНОЕ ОГРАНИЧЕНИЕ; кандидат на
    будущую оптимизацию (покрывающие индексы под ходовые фильтры, кэш/приближённый total, индекс
    под скан athletes/career по имени, лимит конкуренции) — вне 3.5, только если откроем
    массовый глобальный просмотр «вся база с фильтрами».

**Критерий приёмки:** страница результатов (соскоупленная по источнику — реальный UI) открывается
без деградации на 3М строк; повторные запросы идут из кэша. Несоскоупленный глобальный просмотр
под конкуренцией — известный обрыв, см. `full-scan-smoke.js` (за рамками текущего usage).
