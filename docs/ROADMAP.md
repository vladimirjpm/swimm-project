...
- ☐ 11.2.3. **Клиентский шов.** Легаси-дерево `window.normative_*` / `records-helper.ts` с
  ключом `ISR→NR` — под одну страну; новая страница ходит в API напрямую, минуя дерево, и
  не ломает существующие попапы/карточки.
