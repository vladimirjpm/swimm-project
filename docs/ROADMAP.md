...
- ☐ 8.5. Внутренний зачёт группы (переиспользовать механику `ClubPointsRule`), ближайшие
  старты участников.
- ☐ 8.6. Самообслуживание: `CreationPolicy=coach/any` в действии (создание групп не-админом,
  лимит `HubGroupMaxPerUser`), приватность per-group, заявки на вступление.

**Критерий приёмки (8.1–8.4, выполнено):** группа создаётся в админке, публичная страница
показывает состав/рекорды/заплывы, избранное работает как личная группа.
