...
- ✅ 11.1.1. (2026-09-16) **Перечисление стран.** `WorldAquaticsCountriesProvider` за один
  запрос `GET /fina/countries` отдаёт справочник `GUID worldaquatics ↔ alpha-3` — **235
  реальных стран** из 246 записей ответа (11 псевдо-сборных с пустым регионом отсеиваются).
...
1. **Все страны** — все 235 реальных; порог покрытия остаётся вопросом витрины 11.2, не импорта.
