...
| Табы «All programme / My plan» в стартовом протоколе (`StartListViewTab`) | `results-main-project/components/start-list/start-list-tab.tsx` | Не `DeepTabs` сознательно: тот собран как **папка**, сросшаяся с панелью контента (`border-bottom: none`, `margin-bottom: -1px`, парная `.deep-tabs-panel`), а карточка-билет внутрь панели не встаёт — её вырезы-полукруги закрашены фоном СТРАНИЦЫ (`--theme-mode-page-bg`) и на панели превратились бы в чужие кружки. Плюс табу нужен ВТОРОЙ оттенок: план красится цветом избранного (`--theme-personal-*`), а `DeepTabs` знает только акцент. Если такой двухцветный таб понадобится второй раз — заводить проп `tone` в `DeepTabs`, а не третью копию |
