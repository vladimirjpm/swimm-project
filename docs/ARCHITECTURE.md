...
- **Признак сущности ХРАНИТСЯ, а не выводится из названия при показе.** Дисциплина
  соревнования (`Competition.Discipline`, `Sys_DiscoveredCompetitions.Discipline`) —
  свежий пример: догадка по имени срабатывает ОДИН раз (импорт/автозабор), дальше выборки
  фильтруются по колонке. Иначе правило распознавания расползается по вьюхам, новая
  дисциплина требует ещё одной подстроки в разметке, а промах распознавания нельзя
  поправить на одной строке (в админке для этого стоит селект, и повторный забор его не
  затирает). Родня этого правила — перф-урок «фильтр по `.Name` через JOIN убивает индексы,
  резолвить в `Id`»: **имя не идентификатор и не признак**.
- **Материализованный агрегат пересчитывается в ТОЙ ЖЕ операции, что меняет его вход.**
  Материализованы: клубный зачёт (`ClubCompetitionStandings` ← `IClubStandingService`),
  объединённые места (`Results.CombinedPlace` ← `ICompetitionRecalculationService`),
  находки реестра (`Sys_DataCheckFindings` ← прогон). Привязали правило очков, склеили
  клуб, перенесли результаты — сразу зовите пересчёт: без него операция выглядит удачной,
  а цифры на витринах остаются старыми (2026-08-10: привязка правила из находки Health без
  `RebuildForCompetitionAsync` «чинила» находку, не меняя Top Clubs).
...
- **Два законных определения одной величины — два ИМЕНОВАННЫХ понятия, а не переопределение
  на месте.** Сезон: календарный `SeasonMath` (возраст в сезоне, фильтры страниц, ростер,
  `cYear` федерации) и витринный `ShowcaseSeason` (рекорды, season best, KPI клуба —
  режется последним зимним чемпионатом, [season-boundary-rule.md](season-boundary-rule.md)).
  Прецедент внутри самого `SeasonMath` — `StartYearOf` против `FederationYearOf`: «обе
  нумерации законны; ошибка — молча смешать их в одном месте». Правило: заводить соседнее
  понятие со своим именем и явной оговоркой, где какое живёт, а не менять смысл
  существующего — тихая подмена ломает всех потребителей сразу и незаметно.
