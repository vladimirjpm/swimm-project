...
- **Сервер: как сбрасывается** — метки таблиц ставятся сами (К3) и сами же сбрасываются (К4):
  ...
  включена настройка `CacheRowPrecision` ([settings.md](settings.md)). Сужена **страница группы**
  (К4б.4, §3.1): строка группы, участники, подписка, галерея и публикации — по своей группе;
  официальная группа клуба подписки — по клубу (`row:Clubs:{c}`); медиа публикаций — по списку id
  (`row:Sys_UserMedia:{id}`). Правка чужой группы и медиа в My media без публикации её не роняют.
  Список групп и обзоры клубов читают группы таблицей — падают от любой правки группы (обзор
  клуба сузит К4б.5). Места сужения и их сценарии — `CacheRowsSitesTests.Covered`,
  `CacheGroupPageNarrowingTests`.
