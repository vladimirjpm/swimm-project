...
**Не единственный вход.** Тем же `POST /api/admin/loglig/set` привязывают из двух других
мест: находки `/Admin/Health` и **превью затягивания** (карточка «Файл побьёт рекордов» —
см. [competitions.md](competitions.md), «Рекорды в превью»). Логика привязки живёт только
здесь; те экраны лишь зовут этот эндпоинт.
