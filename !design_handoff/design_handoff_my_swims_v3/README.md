...
- Swim rows single-column: medal+⚡ mini-column 20px, label + RELAY, time + place/pts line; right side: 🖼 icon + `▶ N` chip + one status pill per video (stacked).
...
## Reactions (added after the prototype — implement during integration, not in the HTML files)
Not present in the prototypes; add during the React build using the same chip/pill language:
- **Like counter on every media item** (video AND image): `♥ N` chip on each media line in the expanded panel, on competition-media lines, and on Unlinked cards. Muted style like the `🖼 N` chip; `♥ 0` renders dimmer, never hidden (keeps columns aligned). Read-only on this page for the owner (liking happens on public/group pages).
- **Congrats counter on every swim row**: `🎉 N` chip in the swim row (next to the time / before the MEDIA column on desktop; in the time+place line on mobile). On record rows (⚡ PB/CR) the chip is amber-emphasized. Read-only here; tapping to congratulate lives on public pages.
- Group header may aggregate later — NOT in scope now.
- API: swim rows need `congrats_count`, media items need `likes_count` (backed by `Sys_Reactions`: UserId + TargetType(media|result) + TargetId + Kind(like|congrats), unique per user+target).
...
- Accent cyan `#7dd3fc`; action green `#38ef8f` (text on green `#04101f`); warning amber `#ffca7a` (text `#3a2a08`); danger `#ef5350`; private gray `#94a3b8`.
