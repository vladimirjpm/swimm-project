...
## Best swim / Most decorated (утверждено, вариант 4)
Две карточки одинаковой структуры, рядом в левой колонке Overview, каждая делится на ♂ / ♀ (кружок пола 22px: male rgba(29,78,216,.12)/#1d4ed8, female rgba(190,24,93,.12)/#be185d; половина = clickable card, border --theme-mode-border-input, radius 10, padding 12):

- **Best swim of the competition** (метрика — ОЧКИ, не время): в каждой половине круглый медальон 74px (bg color-mix(primary 12%), border 2px color-mix(primary 35%)) с числом очков 24px/900 primary + подпись «points» 9px/800 uppercase; справа имя 15px/800 → пловец, клуб 12.5px/600 → клуб, время 15px/800 tabular-nums, событие+день 11.5px/600 muted.
- **Most decorated**: лого клуба круг 38px, имя 15px/800, клуб 12.5px/600, пилюля медалей «🥇3 🥈1 🥉0» (bg color-mix(primary 15%), text primary).

Логотипы клубов везде приводить к КРУГУ фиксированного размера (26/38/48px, object-fit: cover, border --theme-mode-border); при отсутствии логотипа — инициалы клуба на surface-alt. Разноформатные PNG «в строку» не вставлять.

**RTL:** имя и клуб — всегда на отдельных строках в колонке (не в одну строку), клубу отдана вся свободная ширина; очки в строках HPA — фиксированная колонка 56px, text-align: right; возраст-чип 30px. Это убирает обрезку длинных ивритских названий («…הפועל לב הגליל סחנין»).
