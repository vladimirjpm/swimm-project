...
## Варианты 9a–9f (актуальное направление: мастер-детейл с цветами модулей)

Открой `Competition Overview.dc.html` — секция **9** сверху. Модулей пять, у каждого свой цвет
(см. `MODULE-COLORS.md` + `module-colors.css` — цвет привязан к теме одним токеном на модуль).

| id | Что это | Статус |
| --- | --- | --- |
| **9d** | десктоп 1440: светлая колонка плиток 308px слева + большая карточка справа. Плитка = штрих цвета модуля + медальон-итог + метка + имя лидера + подпись. Активная: заливка 13%, полоса 4px, залитый медальон с белой цифрой/лого, ромб-«язычок» вправо на карточку. | **выбран** |
| **9f** | мобайл 390: то же в аккордеоне — карточка раскрывается ПОД своей плиткой, «язычок» смотрит вниз; шапка brand-blue, тапы ≥44px; HPA сгруппирован по возрасту (♂ и ♀ отдельными строками); персональная полоса — 3 кнопки. | **выбран** |
| 9e | как 9d, но плитки залиты своим цветом (7%) целиком — «цветами» | альтернатива |
| 9a | как 9d, но плитка без медальона-итога (две строки сводки) | история |
| 9b / 9c | «Дорожки»: горизонтальный аккордеон, свёрнутые модули — вертикальные цветные корешки (9c показывает победителя) | отложено |

Итоги на плитках: Top clubs — лого клуба-лидера + рейтинг; New records — число рекордов лидера;
Champions — максимум очков; High Point Award — возрастная группа (8–11) + число наград; Media — счётчик файлов.

Логотип клуба на плитке — круг фиксированного размера (36px десктоп/мобайл, `object-fit: cover`);
при отсутствии логотипа полосатый плейсхолдер в цвете модуля с инициалами (см. `.module-logo`).

State: один `activeModule: 'clubs'|'records'|'champions'|'hpa'|'media'`, в URL — `?tab=overview&panel=records`
(ссылка должна открывать нужную карточку). На мобайле тот же state, но карточка рендерится под плиткой.


### Мобайл 9f — актуальные правила (обновлено)

- Активный таб сжимает плитку в шапку: только кружок с цифрой/лого + НАЗВАНИЕ МОДУЛЯ (имя лидера и подпись скрыты), высота 52px, маленькая палочка скрыта, «язычок» смотрит вниз.
- Внутри карточек нет дублирующего заголовка — ссылка на таб внизу («Все 61 клуб →», «Все 11 рекордсменов →», «Все 8 файлов →», «Все 6 награждённых →»).
- Первый в списке — крупная выделенная строка (круг 54px, метка CHAMPION CLUB / TOP RECORD BREAKER, имя 16–19px, клуб · возраст); остальные обычными строками со 2-го места, ≥44px.
- Единый паттерн блоков ♂/♀: метка по центру, ниже две колонки — слева имя + клуб (align-left), справа итог (медали / время / очки). BEST SWIM — очки крупно в круге 62px.
- High Point: бейдж Age по центру; значок гендера абсолютно центрирован внутри строки (не увеличивает высоту).
- Favorites — отдельный таб-модуль (шестой, только для залогиненного): в 9d/9e и в мобайле 9f. Содержимое: избранные пловцы (инициалы, клуб · возраст, лучшее время, число заплывов, PB), избранный клуб, кнопка «12 избранных заплывов — открыть в Swims».
- В мобайле 9f персональной полосы внизу больше нет — её роль выполняет таб Favorites (My swims / My media остаются на десктопе).
...
## Плитка соревнования + мобильная шапка (новое)
Секция **11** в прототипе — плитка вместо эмодзи 🏆 в hero: сезон + кубок сверху, буква группы (K / M) в центре, возрастная группа снизу. Утверждён вариант **11c**, поставлен в десктопные шапки (1b, 7a) и в мобильную. Секция **10** — фикс мобильной шапки (10a сломано → 10b как надо). Подробности и токены: `COMPETITION-TILE.md` + `competition-tile.css`.

## Files
- `Competition Overview.dc.html` — прототип: блок 2a (мобайл), 1a (отклонён), 1b (утверждён).
- `COMPETITION-TILE.md` + `competition-tile.css` — плитка соревнования 11c и мобильная шапка (токены, размеры, правила данных).
- `NAV-CONTRACT.md` — копия навигационного контракта и токенов.
- `MODULE-COLORS.md` — привязка цветов модулей к темам (что править, чтобы перекрасить всё).
- `module-colors.css` — готовые токены, производные значения и классы плитки/карточки.
- `WIDTH-1920.md` — контейнер 1440 и раскладка на 1920.
- `support.js` — рантайм прототипа (нужен, чтобы .dc.html открывался в браузере).
